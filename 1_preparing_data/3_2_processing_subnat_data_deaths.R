### processing migration data - deaths
library(sf)
library(raster)
library(terra)
library(fasterize)
...
cntryID <- readr::read_csv("../data_in/countries_codes_and_coordinates.csv") %>% 
  dplyr::select(-cntry_code) %>% 
  rename(cntry_code = GADM_code) %>% # use GADM code instead of UN code
  select(cntry_code,iso2,iso3,Country) %>% 
  mutate(iso2 = ifelse(Country == 'Namibia','NB',iso2)) # Namibia's iso2 is NA, problematic to R

cntry_metaData <- read.xlsx("../data_in/cntry_metaData.xlsx",sheet = 'cntry_meta_deaths') %>% 
  as_tibble()
...
GIS_layers <- unique(cntry_metaData$GIS_data)

# number or ratio
NumRat <- cntry_metaData %>% 
...
cntryList <- cntry_metaData %>% 
  filter(GIS_data == GIS_layers[4]) %>% 
...
subnat_data <- readr::read_csv('../data_out/oecd_deaths_tl2_data.csv') %>% 
  rename(Subnat = Region) %>% 
  dplyr::filter(iso3 %in% cntryList$GID_0) %>% 
  arrange(iso3) %>% 
  select(-c(TL,Subnat,iso2,Country,cntry_code))

#names(subnat_data) <- c('tl2_id','iso3',as.character(2001:2019))

id_subnat <- unique(subnat_data$iso3)

gis_data <- read_sf( '../data_in_gpkg/oecd_tl2.gpkg' ) %>% 
  #st_drop_geometry() %>% 
  select(tl2_id,iso3,name_en) %>% 
  rename(Subnat = name_en) %>% 
  left_join(cntryID,by='iso3') %>% 
  select(Country,Subnat,iso3,cntry_code,tl2_id) %>% 
  dplyr::filter(iso3 %in% cntryList$GID_0) %>% 
  arrange(iso3)
...
subnat_gis_data <- gis_data %>% 
  ...
  select(Country,iso3,cntry_code,Subnat,GID_1,GID_nmbr,NumRate,as.character(2001:2019))
...
subnat_data <- read.xlsx('../data_in/death_rate_v3.xlsx', sheet='death_rate', startRow = 8) %>% 
  as_tibble() %>% 
  rename(iso3=GID_0) %>% 
  select(-GID_1) %>% 
...
subnat_gis_data %>% filter(GID_1 == 'RO31')
  
temp <- subnat_gis_data_EUROSTAT %>% 
  st_drop_geometry()

### individual countries ----

cntryList <- cntry_metaData %>% 
  filter(GIS_data %in% GIS_layers[5:8]) %>% 
  select(GID_0,ID_0,NAME_0,NUMBER_RATE,Notes) 
...
# temp_gis_BHS <- read_sf( '../data_in_gpkg/GIS extra/Bahamas.gpkg') %>% 
...
temp_gis_TUN <- read_sf( '../data_in_gpkg/GIS extra/tunisia_OECD_v2.gpkg') %>% 
  select(GDLcode,region,iso_code) %>% 
  rename(iso3 = iso_code) %>% 
  rename(Subnat = region) %>% 
  rename(GID_1 = GDLcode) %>% 
  left_join(cntryID,by='iso3') %>% 
  select(Country,Subnat,iso3,cntry_code,GID_1)

# ts_list <- list(objects(pattern = "temp_gis_"))
# map_df(ts_list, as_tibble)
# 
# gis_data <- bind_rows(map_df)

gis_data <- bind_rows(#temp_gis_BHS, 
                      temp_gis_LBN,
                      temp_gis_PRK,
                      temp_gis_TUN) %>% 
  select(Country,iso3,cntry_code,GID_1) %>% 
...
# de_ind = gis_data[st_is_empty(gis_data),,drop=FALSE] %>% 
#   st_drop_geometry()
# view(de_ind)
...
# de_ind = subnat_gis_data_individuals[st_is_empty(subnat_gis_data_individuals),,drop=FALSE] %>% 
#   st_drop_geometry()
# view(de_ind)
### combine subnat data ---

#st_collection_extract(subnat_gis_data_GADM,type = "POLYGON")

# test <- subnat_gis_data_EUROSTAT %>% 
#   filter(GID_1 == 'DED')
...
names(subnat_gis_combined) <- c('Country', 'iso3', 'cntry_code', 'Subnat', 'GID_1', 'GID_nmbr', 'NumRate',
                                paste0("deaths", as.character(1990:2019)), 'geom' )

st_write(subnat_gis_combined, "../results/test_deaths_combined.gpkg",delete_dsn = TRUE)
...
write_csv(temp, "../results/deaths_subnat_combined_v2.csv")


subnat_gis_combined <- read_sf( "../results/test_deaths_combined.gpkg" )
...
subnat_data_combined <- read_csv("../results/deaths_subnat_combined_v2.csv")
names(subnat_data_combined) <- c("Country", "iso3","cntry_code","Subnat","GID_1","GID_nmbr","NumRate",paste0("ratio",1990:2019))


# join tables together
subnat_gis_combined_pop <- subnat_gis_combined %>% 
...
write_csv(temp, "../results/test_deaths_combined_pop_ratio_v2.csv")



### fill missing data ----

subnat_gis_combined_pop <- read.csv( "../results/test_deaths_combined_pop_ratio_v2.csv" )
...
# read missing data
listMissingData <- read_csv("../data_in/missing_data_list_deaths.csv") %>% 
...
write_csv(subnat_gis_combined_ratio, "../results/deaths_ratio_filled.csv")
